  ...
  const ledgerExplainQuery = trpc.aiExplain.ledgerMonth.useQuery(
    {
      ...
      month: {
        ...
        // Round 96 — keep the two cash-in legs SEPARATE so the explainer can
        // distinguish a Treasury (CBK) maturity from a maturing bank deposit.
        cbkCashIn: explainRow?.cbkCashIn ?? 0,
        bankCashIn: explainRow?.bankCashIn ?? 0,
        mmfToDhow: explainRow?.mmfToDhow ?? 0,
        mainAction: explainRow?.mainAction ?? null,
        mmfEndBalance: explainRow?.mmfEnd ?? 0,
        secondaryMmfEndBalance: explainRow?.secondaryMmfEnd ?? 0,
        tbillEndBalance: (explainRow?.tbill91End ?? 0) + (explainRow?.tbill182End ?? 0) + (explainRow?.tbill364End ?? 0),
        tbill91EndBalance: explainRow?.tbill91End ?? 0,
        tbill182EndBalance: explainRow?.tbill182End ?? 0,
        tbill364EndBalance: explainRow?.tbill364End ?? 0,
        ifbEndBalance: explainRow?.ifbEnd ?? 0,
        fxdEndBalance: explainRow?.fxdEnd ?? 0,
        bankEndBalance: explainRow?.bankEnd ?? 0,
