...
describe("Stage 10b-1 · B — ResearchDesk.tsx: multi-field edit path extended to Bank", () => {
  it("4. EditCatalogueFieldsDialog now supports both mmf and bank, with per-catalogue envelope routing", () => {
    ...
    // Stage 10b-2 extended this further to also support "cbk", and Stage
    // 10b-3 extended it again to "market_asset" — anchor updated to match.
  ...
  it("the Edit fields entry points (pending card + approval modal) are both gated to mmf OR bank (OR cbk, Stage 10b-2; OR market_asset, Stage 10b-3)", () => {
