...
// ─── Pending Rate Fetches ─────────────────────────────────────────────────────

import {
  pendingRateFetches,
  rateFetchLog,
  type InsertPendingRateFetch,
  type InsertRateFetchLog,
} from "../drizzle/schema";

export async function getPendingRateFetches(userId: number) {
  const db = await getDb();
  if (!db) return [];
  return db
    .select()
    .from(pendingRateFetches)
    .where(and(eq(pendingRateFetches.userId, userId), eq(pendingRateFetches.status, "pending")))
    .orderBy(desc(pendingRateFetches.fetchedAt));
}

export async function insertPendingRateFetch(data: InsertPendingRateFetch) {
  const db = await getDb();
  if (!db) return;
  await db.insert(pendingRateFetches).values(data);
}

export async function updatePendingRateFetchStatus(
  id: number,
  userId: number,
  ...
  if (!db) return;
  await db
    .update(pendingRateFetches)
    .set({ status })
    .where(and(eq(pendingRateFetches.id, id), eq(pendingRateFetches.userId, userId)));
}

/** Dismiss all pending fetches for a user (e.g. on "dismiss all"). */
export async function dismissAllPendingRateFetches(userId: number) {
  const db = await getDb();
  if (!db) return;
  await db
    .update(pendingRateFetches)
...
export async function insertRateFetchLog(data: InsertRateFetchLog) {
  const db = await getDb();
  if (!db) return;
  await db.insert(rateFetchLog).values(data);
}

/** Get the most recent successful fetch log per source for staleness display. */
export async function getLastSuccessfulFetches(userId: number) {
  const db = await getDb();
  if (!db) return [];
  // Get the most recent successful fetch for each source
  const rows = await db
    .select()
    .from(rateFetchLog)
    .where(and(eq(rateFetchLog.userId, userId), eq(rateFetchLog.success, true)))
...
/** Get the most recent fetch log (success or failure) per source. */
export async function getLastFetchAttempts(userId: number) {
  const db = await getDb();
  if (!db) return [];
  const rows = await db
    .select()
    .from(rateFetchLog)
    .where(eq(rateFetchLog.userId, userId))
    .orderBy(desc(rateFetchLog.fetchedAt))
    .limit(20);
  const seen = new Set<string>();
  return rows.filter((r) => {
